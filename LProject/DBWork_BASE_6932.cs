...
namespace LProject
{
    ...
    class DBWork
    {
        ...
        public DBWork()
        {
            db = new DataContext("Data Source=DESKTOP-HF45CKO\\SQLEXPRESS;Initial Catalog=LProject;Integrated Security=True;Pooling=False");
        ...
        public List<PointStruct> getPointBySession(int sessionID)
        {
            ...
            var res = point.Where(f => f.ID_Session == sessionID).Select(data =>
                new PointStruct { city = data.City, street = data.Street, house = data.House, korp = data.Korp, interval = data.Interval, routeId = data.ID_Route })
                .Distinct().ToArray();
